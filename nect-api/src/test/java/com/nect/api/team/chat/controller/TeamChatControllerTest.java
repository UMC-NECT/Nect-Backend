...
public class TeamChatControllerTest {
    ...
    void getProjectMembers() throws Exception {
        ...
        mockMvc.perform(get("/chats/rooms/{projectId}/users", 1L)
    ...
    void createPersonalChatRoom() throws Exception {
        ...
        mockMvc.perform(post("/chats/rooms/personal")
                        .contentType(MediaType.APPLICATION_JSON)
                ...
                .andDo(document("chat-room-create-personal",
                        ...
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .requestFields(
                                        ...
                                        fieldWithPath("target_user_id").description("대화 상대 유저 ID")
                                )
                                .responseFields(
                                        ...
                                        fieldWithPath("body.room_name").description("채팅방 이름 (상대방 이름)"),
                                        fieldWithPath("body.room_type").description("채팅방 타입 (DIRECT)"),
                                        fieldWithPath("body.profile_image").description("채팅방 이미지 (상대방 프사)").optional(),
    ...
    void createGroupChatRoom() throws Exception {
        ...
        mockMvc.perform(post("/chats/rooms/group")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andDo(document("chat-room-create-group",
                        ...
                        resource(ResourceSnippetParameters.builder()
                                .tag("채팅방 생성")
                                .summary("그룹 채팅방 생성")
                                .description("그룹 채팅방을 생성합니다. (채팅방 이름 공백 불가)")
                                .requestFields(
                                        fieldWithPath("project_id").description("프로젝트 ID"),
                                        fieldWithPath("room_name").description("채팅방 이름"),
                                        fieldWithPath("target_user_ids").description("초대할 유저 ID 리스트")
                                )
                                .responseFields(
